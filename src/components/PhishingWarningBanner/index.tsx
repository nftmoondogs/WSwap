...
const Container = styled(Flex)`
  ...
  background: linear-gradient(0deg, rgba(39, 38, 44, 0.4), rgba(39, 38, 44, 0.4)),
    linear-gradient(180deg, #09070c 22%, #5b5ff7 100%);
  ${({ theme }) => theme.mediaQueries.md} {
    padding: 0px;
    background: linear-gradient(180deg, #09070c 22%, #32359f 100%);
  }
`
...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t("please make sure you're visiting https://www.woofswap.co/ - check the URL carefully.")
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://www.woofswap.co/'}
          color={text === 'https://www.woofswap.co/' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
    ...
    <Container className="warning-banner">
      {isMobile || isMd ? (
        ...
        <>
          <InnerContainer>
            <img
              src="/images/decorations/woof-img-light.png"
