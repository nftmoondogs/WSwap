...
export interface SalesSectionProps {
  ...
  additionalButton?: SalesSectionButton
  images: CompositeImageProps
}

const SalesSection: React.FC<React.PropsWithChildren<SalesSectionProps>> = (props) => {
  const { headingText, bodyText, reverse, primaryButton, secondaryButton, additionalButton, images } = props
      ...
      >
        ...
        >
          ...
          <Flex>
            ...
            {additionalButton && ( // Check if additionalButton is defined before rendering
              <Button mr="16px">
                {additionalButton.external ? (
                  <Link external href={additionalButton.to}>
                    <Text color="card" bold fontSize="16px">
                      {additionalButton.text}
                    </Text>
                  </Link>
                ) : (
                  <RouterLink to={additionalButton.to}>
                    <Text color="card" bold fontSize="16px">
                      {additionalButton.text}
                    </Text>
                  </RouterLink>
