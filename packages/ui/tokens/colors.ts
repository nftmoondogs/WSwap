export const baseColors = {
  white: 'white',
  failure: '#ED4B9E',
  failure33: '#ED4B9E33',
  primary: '#366dff',
  primary0f: '#1FC7D40f',
  primary3D: '#1FC7D43D',
  primaryBright: '#53DEE9',
  primaryDark: '#0098A1',
  success: '#366dff',
  success19: '#31D0AA19',
  warning: '#366dff',
  warning2D: '#ED4B9E2D',
  warning33: '#ED4B9E33',
}

export const additionalColors = {
  binance: '#F0B90B',
  overlay: '#452a7a',
  gold: '#FFC700',
  silver: '#B2B2B2',
  bronze: '#E7974D',
}

export const lightColors = {
  ...baseColors,
  ...additionalColors,
  secondary: '#FFD700',
  secondary80: '#9A6AFF80',
  background: '#08060B',
  backgroundDisabled: '#3c3742',
  backgroundAlt: '#2A2448',
  backgroundAlt2: 'rgba(39, 38, 44, 0.7)',
  cardBorder: '#9D00FF',
  contrast: '#FFFFFF',
  dropdown: '#1E1D20',
  dropdownDeep: '#100C18',
  invertedContrast: '#191326',
  input: '#372F47',
  inputSecondary: '#262130',
  primaryDark: '#0098A1',
  tertiary: '#353547',
  text: '#F4EEFF',
  text99: '#F4EEFF99',
  textDisabled: '#666171',
  textSubtle: '#B8ADD2',
  disabled: '#524B63',
  Herocolour: '#FFFFFF',
  gradientBubblegum: 'linear-gradient(139.73deg, #313D5C 0%, #3D2A54 100%)',
  gradientInverseBubblegum: 'linear-gradient(139.73deg, #3D2A54 0%, #313D5C 100%)',
  gradientCardHeader: 'linear-gradient(136.77deg, #9370DB 0%, #B87333 100%)',
  gradientBlue: 'linear-gradient(180deg, #00707F 0%, #19778C 100%)',
  gradientViolet: 'linear-gradient(180deg, #6C4999 0%, #6D4DB2 100%)',
  gradientVioletAlt: 'linear-gradient(180deg, #434575 0%, #66578D 100%)',
  gradientGold: 'linear-gradient(180deg, #FFD800 0%, #FDAB32 100%)',
  gradientBold: 'linear-gradient(#53DEE9, #9A6AFF)',
  brandcolour: 'linear-gradient(180deg, #09070c 22%, #9370DB 100%)',
  backgroundImage: `radial-gradient(103.12%50%at 50%50%,#21193a 0%,#191326 100%)`,
  backgroundBridge: `url(https://raw.githubusercontent.com/SVerseLab/images/master/BG_EDITTED.png)`,
  HeaderImage: `url(https://raw.githubusercontent.com/SVerseLab/images/master/netbackground.png)`,
  backgroundImageDesktop: `url(https://raw.githubusercontent.com/SVerseLab/images/master/mobile-background.png)`,
  coneDesktop: `url(https://raw.githubusercontent.com/SVerseLab/images/master/coneback.png)`,
}

export const darkColors = {
  ...baseColors,
  ...additionalColors,
  secondary: '#5d8fff',
  secondary80: '#9A6AFF80',
  background: '#08060B',
  backgroundDisabled: '#3c3742',
  backgroundAlt: '#1d1c21',
  backgroundAlt2: 'rgba(39, 38, 44, 0.7)',
  cardBorder: '#2b2b2b',
  contrast: '#FFFFFF',
  dropdown: '#1E1D20',
  dropdownDeep: '#100C18',
  invertedContrast: '#191326',
  input: '#2f3047',
  inputSecondary: '#212230',
  primaryDark: '#0098A1',
  tertiary: '#353547',
  text: '#F4EEFF',
  text99: '#F4EEFF99',
  textDisabled: '#666171',
  textSubtle: '#B8ADD2',
  disabled: '#524B63',
  Herocolour: '#FFFFFF',
  gradientBubblegum: 'linear-gradient(139.73deg, #313D5C 0%, #2a2d54 100%)',
  gradientInverseBubblegum: 'linear-gradient(139.73deg, #3D2A54 0%, #313D5C 100%)',
  gradientCardHeader: 'linear-gradient(166.77deg,#3b4155,#303445)',
  gradientBlue: 'linear-gradient(180deg, #00707F 0%, #19778C 100%)',
  gradientViolet: 'linear-gradient(180deg, #6C4999 0%, #6D4DB2 100%)',
  gradientVioletAlt: 'linear-gradient(180deg, #434575 0%, #66578D 100%)',
  gradientGold: 'linear-gradient(180deg, #FFD800 0%, #FDAB32 100%)',
  gradientBold: 'linear-gradient(#53DEE9, #9A6AFF)',
  brandcolour: 'linear-gradient(180deg, #09070c 22%, #9370DB 100%)',
  backgroundImage: `radial-gradient(103.12%50%at 50%50%,#313d5c 0%,#131426 100%)`,
  backgroundBridge: `url(https://raw.githubusercontent.com/SVerseLab/images/master/BG_EDITTED.png)`,
  HeaderImage: `url(https://raw.githubusercontent.com/SVerseLab/images/master/netbackground.png)`,
  backgroundImageDesktop: `url(https://i.ibb.co/zrGrtXp/mobile-backgroundwoof.png)`,
  coneDesktop: `url(https://raw.githubusercontent.com/SVerseLab/images/master/coneback.png)`,
}
